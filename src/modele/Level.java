...
public class Level {
	...
	public Level(int[][] maze, int player_row, int player_col, LevelSelectorState levelSelectorState){
		...
		if(ID == 1)
			...
			solved = false;
		xOffset = (Jeu.SIZE_X - maze[0].length*TILESIZE)/2;
		yOffset = (Jeu.SIZE_Y - maze.length*TILESIZE)/2;
		texture = Assets.PlayerFront;
		restart = new Button("RESTART", 100, Jeu.SIZE_Y/2, new Click(){
			...
			public void onClick() {
				...
				Assets.font30);
		back = new Button("BACK", Jeu.SIZE_X - 100, Jeu.SIZE_Y/2, new Click(){
