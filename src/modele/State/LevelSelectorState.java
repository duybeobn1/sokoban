...
public class LevelSelectorState extends State{
	...
	private final int xOffset = (Jeu.SIZE_X - DOUBLETILESIZE*6)/2;
	private final int yOffset = (Jeu.SIZE_Y - DOUBLETILESIZE*5)/2;
	
	private Button back;
	
	public LevelSelectorState(Jeu jeu) {
		super(jeu);
		
		for(int i = 0; i < levels.length; i++)
			levels[i] = loadLevel("../../res/levels/"+i+".txt");
		
		back = new Button("BACK", Jeu.SIZE_X/2, Jeu.SIZE_Y - 100, new Click(){
			...
			for(int j = 0; j < 6; j++){
				...
				if(bounds.contains(MouseManager.x, MouseManager.y)){
					if(MouseManager.left && levels[counter-1].isSolved()){
						((GameState)jeu.getGameState()).setLevel(levels[counter-1]);
						State.currentState = jeu.getGameState();
					}
